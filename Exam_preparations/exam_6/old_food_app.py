...
class FoodOrdersApp:
    ...
    def add_meals_to_menu(self, *meals: Meal):
        for meal in meals:
            if meal.__class__.__name__ in self.VALID_MEALS:
                ...
                if meal.name not in self.meals_dict:
                    self.meals_dict[meal.name] = 0
                self.meals_dict[meal.name] += meal.quantity
    ...
    def __validate_if_all_meals_exist(self, meal_names_and_quantities):
        for meal in meal_names_and_quantities.keys():
            if meal not in self.meals_dict:
    ...
    def __validate_if_there_is_enough_quantity(self, meal_names_and_quantities):
        for name, quantity in meal_names_and_quantities.items():
            if quantity > self.meals_dict[name]:
                meal = self.__get_meal_by_name(name)
